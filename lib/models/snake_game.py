...
class SnakeGame:
    ...
    def __init__(self, player:SnakePlayer, size: Tuple[int, int]=(12, 12), display=None):
        self.map_width = size[0]
        self.map_height = size[1]
        self.game_board = np.full((self.map_width, self.map_height), SnakeGame.EMPTY_SQUARE, dtype=int)
    ...
    def step(self):
        game_state = GameState(self.game_board, self.snake_pos, self.snake_size, self.died)
        ...
        self.game_board[self.snake_pos.y, self.snake_pos.x] = self.snake_size + 1
    ...
    def get_random_apple_location(self):
        ...
        if zero_positions:
            selected_position = zero_positions[np.random.choice(len(zero_positions))]
